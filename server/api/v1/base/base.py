...
class BaseQuery(object):
    ...
    def _calculation_extra(self, data):
        """
        额外补充查询
        :param data:
        :return:
    ...
    def _get(self, key):
        return self.__model__.query.filter_by(id=key).first()
        ...
        if data:
            if isinstance(data, (list, tuple)):
                ...
                # 根据keys定义查询多信息
                    ...
                    data = list(map(lambda x: {p.key: getattr(x, p.key)
                                               for p in self.__model__.__mapper__.iterate_properties if
                                               p.key in search_key_list
                                               }, data))

            else:
                if not search_key_list:
                    print(
                        f"mapper.iterate_properties的内容为：{self.__model__.__mapper__.iterate_properties}, type is {type(self.__model__.__mapper__.iterate_properties)}")
    ...
    def _parse_query_field(self):
        ...
        for query_key, query_value in args.items():
            ...
            if not self._check_key(key=key):
                continue
            # if query_key == "ge-create_time":
            #     query_value += " 00:00:00"
            # if query_key == "le-create_time":
...
class Service(BaseParse, BaseQuery, MethodView):
    ...
    def get(self, key=None):
        ...
        if key is not None:
            print("...................")
            print("self._get(key=key)的结果为：+++++++++>", self._get(key=key), type(self._get(key=key)))
            data = self.parse_data(self._get(key=key))
            if data:
                res.update(result=data)
            else:
                res.update(returncode=ResponseCode.NoResourceFound, message=ResponseMessage.NotFondResourceErr)  # 未找到资源
        else:
            ...
            if data:
                data = self._calculation_extra(data)
                res.update(result=data)
            else:
                res.update(returncode=ResponseCode.NoResourceFound, message=ResponseMessage.NotFondResourceErr)  # 未找到资源
    ...
    def post(self):
        ...
        """
        res = ResponseMsg()
        data = self._parse_create_field()  # type: list
        if data:
